...
/* count_observations(filename)
   Given a filename, count and return the number of complete temperature 
   observations in the file. If the file cannot be opened, return -1.
...
/* load_all_observations(filename, array_size, observation_array)
   Given a filename and an array of Observation objects, read as many complete
   observations from the file as possible (up to the size of the array) and
   store each observation (in order) into the array.

   If the file cannot be opened, return -1. Otherwise, return the number of 
   complete observations read.

   If the file contains fewer complete observations then there are elements in the
   array, stop reading after the last complete observation in the file. Otherwise,
   stop reading once array_size observations are read.
   ...
   Return value (integer):
     ...
     - Otherwise, return the total number of complete observations read and stored
       into the array.
*/
